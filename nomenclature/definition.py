...
class DataStructureDefinition:
    ...
    def get_invalid_units(self, df: IamDataFrame) -> pd.DataFrame:
        """Get a DataFrame of variables with invalid units
        ...
        :class:`pandas.DataFrame`
            DataFrame with columns *variable*, *unit*, and *expected*.
        """
        ###
        # METHOD BODY BELOW GENERATED BY COPILOT, CHECK!!
        ###
        lst = []
        for variable, unit in df.unit_mapping.items():
            if variable in self.variable:
                dsd_unit = self.variable[variable].unit
                # fast-pass for unique units in df and the DataStructureDefinition
                if dsd_unit == unit:
                    continue
                # full-fledged subset validation
                if CodeList.is_subset(unit, dsd_unit):
                    continue
                lst.append((variable, unit, dsd_unit))

        if lst:
            return pd.DataFrame(
                lst,
                columns=["variable", "unit", "expected"],
                index=[v for v, _, _ in lst],
            )
